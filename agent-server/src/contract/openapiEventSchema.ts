 ...
 * than authored as zod. `buildOpenApiDocument` below is the single entry point
 * used by both the static `openapi` dump and the live `/openapi.json` handler,
 * so the published metadata and merged schema can't drift between them.
 *
 * TODO: Long-term agent-server should define its own Anti-Corruption Layer to
...
/**
 * Build the full OpenAPI 3.1 document for a mounted route tree: the canonical
 * {@link OPENAPI_INFO}, the REST surface (from `@hono/zod-openapi`), and the
 * merged typia-generated SSE wire-event schema. The single source of truth for
 * the document shape — both `src/contract/openapi.ts` (static dump) and
