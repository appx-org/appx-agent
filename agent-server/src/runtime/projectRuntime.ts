...
export class ProjectRuntime {
  ...
  static async create(config: ProjectRuntimeConfig): Promise<ProjectRuntime> {
    ...
    const { systemPrompt, agentsFilePath } = resolveSystemPrompt(
      config,
      projectDir,
      logger,
    );
  ...
  private sessionModelDefaults(): Pick<
  ...
  > {
    ...
    if (this.defaultModelProvider && this.defaultModelId) {
      ...
      if (model) {
        ...
        const thinkingLevel = this.credentials.defaultThinkingForModel(
...
function resolveSessionsDir(
  ...
  if (config.sessionsDir) {
    return isAbsolute(config.sessionsDir)
      ? config.sessionsDir
      : resolve(projectDir, config.sessionsDir);
  }
...
function resolveSystemPrompt(
  ...
  const conventionPath = resolve(
    projectDir,
    PROJECT_PI_DIR,
    CONVENTION_AGENTS_FILE,
  );
