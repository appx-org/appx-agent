...
export class AgentRuntime {
    ...
    constructor(config) {
        ...
        if (config.agentsFile) {
            ...
            try {
                ...
                this.logger.log(`[agent] system prompt loaded from ${path} (${this.systemPrompt.length} chars)`);
            }
            catch (err) {
        ...
        if (config.anthropicApiKey) {
            ...
            this.logger.log("[agent] no ANTHROPIC_API_KEY provided; relying on AuthStorage defaults (~/.pi/agent/auth.json)");
        }
    }
    /**
     ...
     */
    async makeResourceLoader() {
        const settingsManager = SettingsManager.create(this.projectDir, getAgentDir());
     ...
     */
    async listSessions() {
        const list = await SessionManager.list(this.projectDir, this.sessionsDir);
