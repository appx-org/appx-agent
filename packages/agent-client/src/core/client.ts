 ...
 * the generated `paths` (from `@appx-org/agent-protocol`). That means request
...
export class AgentClient {
	...
	async getSessionMessages(projectId: string, sessionId: string): Promise<{ id: string; messages: AgentMessage[] }> {
		return this.unwrap(
	...
	async getSessionSettings(projectId: string, sessionId: string): Promise<AgentSessionModelSettings> {
		return this.unwrap(
	...
	async listExtensionUiRequests(projectId: string, sessionId: string): Promise<{ requests: ExtensionUiRequest[] }> {
		return this.unwrap(
	...
	async respondExtensionUiRequest(
		...
		return this.unwrap(
			await this.http.POST("/v1/projects/{projectId}/sessions/{id}/extension-ui/{requestId}/response", {
