...
function contentText(content: unknown): string {
	...
	return content
		.map((block) =>
			(block as { type?: string }).type === "text" ? String((block as { text?: unknown }).text ?? "") : "",
		)
		.join("");
}
...
function latestAssistantIndex(messages: UiMessage[], requireStreaming: boolean): number {
	for (let i = messages.length - 1; i >= 0; i--) {
		...
		if (message && message.role === "assistant" && (!requireStreaming || message.streaming)) return i;
	}
	return -1;
}
...
function findToolPartIndex(parts: UiMessagePart[], toolCallId: string, contentIndex?: number): number {
	return parts.findIndex(
		(p) =>
			p.type === "tool" &&
			((toolCallId && p.id === toolCallId) || (typeof contentIndex === "number" && p.contentIndex === contentIndex)),
	);
}
 ...
 */
function applyAssistantSnapshot(messages: UiMessage[], partial: AssistantMessagePartial): UiMessage[] {
...
function reduceEvent(state: SessionState, event: AgentEvent): SessionState {
	switch (event.type) {
		...
		case "agent_end": {
			...
			const runError = !event.willRetry && lastAssistant ? assistantErrorMessage(lastAssistant) : null;
			return {
				...state,
				status: "idle",
