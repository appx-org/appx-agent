...
export class SessionStore {
	...
	private async refreshExtensionRequests(projectId: string, sessionId: string, entryKey: string): Promise<void> {
		const entry = this.entries.get(entryKey);
	 ...
	 */
	async loadModelSettings(projectId: string, sessionId: string, options?: { force?: boolean }): Promise<void> {
		const entryKey = SessionStore.key(projectId, sessionId);
		this.attach(projectId, sessionId);
		const entry = this.entries.get(entryKey);
