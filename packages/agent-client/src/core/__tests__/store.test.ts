...
describe("SessionStore — sendPrompt", () => {
	it("adds an optimistic user bubble and POSTs the prompt", async () => {
		...
		const promptPost = requests.find((request) => request.method === "POST" && request.path.endsWith("/prompt"));
...
describe("SessionStore — abort", () => {
	const countMessageGets = () =>
		requests.filter((request) => request.method === "GET" && request.path === "/v1/projects/proj/sessions/sess")
			.length;
...
describe("SessionStore — model settings", () => {
	...
	it("loads models + settings once into shared state and skips redundant fetches", async () => {
		...
		const settingsGets = () => requests.filter((r) => r.method === "GET" && r.path.endsWith("/settings")).length;
