...
export function Markdown({ text, streaming = false, className }: MarkdownProps) {
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies(html): `html` is consumed via dangerouslySetInnerHTML, so this effect must re-run after each render it triggers to decorate the fresh DOM.
	useEffect(() => {
	...
	return (
		<div
			...
			// biome-ignore lint/security/noDangerouslySetInnerHtml: `html` is produced by renderMarkdown, which sanitizes through DOMPurify.
