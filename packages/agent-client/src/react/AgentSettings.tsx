...
export function AgentSettings({
	...
	const loadProviders = useCallback(async () => {
		...
		try {
			const [authRes, customRes] = await Promise.all([client.listAuthProviders(), client.listCustomProviders()]);
	...
	const subscriptionNeedsInput =
		subscriptionFlow?.status === "prompt" || (subscriptionFlow?.status === "auth" && subscriptionFallbackOpen);
	...
	const hasOpenEditorProvider = customProviders.some((provider) => provider.provider === customForm.provider.trim());
	const editorApiKeyMissing =
		!customForm.apiKey.trim() &&
		!customProviders.find((provider) => provider.provider === customForm.provider.trim())?.apiKeyConfigured;
			...
			<div className="agent-settings-card">
				...
				{selected && (
					<div className="agent-settings-provider-meta">
						...
						<span className={selected.configured ? "agent-settings-configured" : "agent-settings-unset"}>
				...
				{showSubscriptionControls && subscriptionFlow && (
					<div className="agent-settings-flow-panel">
						<div className="agent-settings-flow-header">
							...
							<span className={cx("agent-settings-flow-pill", flowStatusModifier(subscriptionFlow.status))}>
						...
						{subscriptionFlow.error && <div className="agent-settings-error">{subscriptionFlow.error}</div>}
				...
				<div className="agent-settings-provider-list">
					{sortedProviders.length === 0 ? (
						...
						sortedProviders.slice(0, 14).map((provider) => (
							...
							>
								<span className="agent-settings-provider-name">{provider.name || provider.provider}</span>
								<span className={provider.configured ? "agent-settings-configured" : "agent-settings-unset"}>
				...
				<div className="agent-settings-section">
					<div className="agent-settings-section-header">
						<div className="agent-settings-section-title-group">
							<h3 className="agent-settings-card-title agent-settings-section-title">Custom Provider</h3>
							<span className="agent-settings-muted">
							...
							{customProviders.map((provider) => (
								<div key={provider.provider} className="agent-settings-custom-row">
									...
									>
										...
										<span
											className={
												provider.apiKeyConfigured ? "agent-settings-configured" : "agent-settings-unset"
											}
										>
						...
						<div className="agent-settings-custom-editor">
							...
							<div className="agent-settings-toggle-row">
								...
								<select
									...
									onChange={(event) =>
										updateCustomForm("thinkingPreset", event.target.value as CustomForm["thinkingPreset"])
									}
