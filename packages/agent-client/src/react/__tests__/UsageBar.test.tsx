...
describe("UsageBar", () => {
	it("renders cost, cache-hit and context values", () => {
		...
		const values = Array.from(container.querySelectorAll(".agent-chat-usage-value")).map((node) => node.textContent);
	...
	it("honours custom labels", () => {
		const { container } = render(
			<UsageBar metrics={metrics()} labels={{ cost: "Kosten", cache: "Cache", context: "Kontext" }} />,
		);
		const labels = Array.from(container.querySelectorAll(".agent-chat-usage-label")).map((node) => node.textContent);
